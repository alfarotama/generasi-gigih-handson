...
function Search() {
	...
	async function handleClick() {
		try {
			set_data_img(null);
			await axios.get(url_giphy).then((res) => {
		...
		<>
			<div className="">
				<input
					...
					type="text"
					className="bg-gray-200 px-4 py-3 rounded-bl-lg rounded-tl-lg w-11/12"
				...
				<button
					...
					className="bg-blue-400 text-white px-4 py-3 rounded-br-lg rounded-tr-lg font-bold w-1/12 hover:bg-yellow-500"
				>
			...
			{data_img && (
				<div className="bg-gray-100 p-8 rounded-lg mt-5 flex flex-wrap">
					{data_img.map((item) => {
						return <Gif url={item.images.original.url} title={item.title} />;
					})}
				</div>
			)}
		</>
